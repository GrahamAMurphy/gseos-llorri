...
def fBoot(oSeq):
  ...
  fCheckPacket(oSeq, bBootStatus, lambda: (bBootStatus.MemWrite==1))
  fCheckCmdCounts(oSeq)
  C('---Load a file: messfor.mem---')
  fLoadFile('/jrh/messfor.mem', 0)
  oSeq.Sleep(15)			# TBD: wait for load to complete
  fSyncCmd()				# sync up command counters
  C('---MEM_CHECK used to verify successful load---')
  fSendCmd('MEM_CHECK', 0, 6150)
  fGoodCmd()
# fCheckEcho(oSeq, 'MEM_CHECK', 0x00,	# there is a race between echo and check
#		[0x00, tstpg, 0x00, 0x00, 0x18, 0x06])
  fCheckMemCheck(oSeq, 0, 6150, 0x8a02)
  fCheckCmdCounts(oSeq)
  C('---MEM_RUN used to start program---')
  fSendCmd('MEM_RUN', 0x42a)		# no echo
  ...
  fCheckPacket(oSeq, bBootStatus, lambda: (bBootStatus.MemWrite==1))
  fCheckCmdCounts(oSeq)
  C('---Load a file: messfore.mem---')
  fLoadFile('/jrh/messfore.mem', 0)
  oSeq.Sleep(15)			# TBD: wait for load to complete
  fSyncCmd()				# sync up command counters
  C('---MEM_CHECK used to verify successful load---')
  fSendCmd('MEM_CHECK', 0, 6155)
  fGoodCmd()
# fCheckEcho(oSeq, 'MEM_CHECK', 0x00,	# there is a race between echo and check
#		[0x00, tstpg, 0x00, 0x00, 0x18, 0x06])
  fCheckMemCheck(oSeq, 0, 6155, 0x8269)
  fCheckCmdCounts(oSeq)
...
GseosSequencer.TSequencer('Boot Code Regression', fRegression)
