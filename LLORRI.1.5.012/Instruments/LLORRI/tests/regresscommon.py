...
# ========================================================================
# Memory extension - enable/disable NV memory
# ========================================================================

def fMemoryExtend(oSeq,):
  C('---Memory extension---')
  C('---Checking defaults---')
  fCheckPacket(oSeq, bStatus, lambda: (bStatus.NVMem_Enb==0) )
  C('---MEM_NV_ENB(Sleep/Reset)---')
  fSendCmd('MEM_NV_ENB', 'Sleep/Reset')
  fCheckEcho(oSeq, 'MEM_NV_ENB', 0, [1])
  fGoodCmd()
  fCheckPacket(oSeq, bStatus, lambda: (bStatus.NVMem_Enb==1) )
  C('---MEM_NV_ENB(Enable)---')
...
#		[0x00, tstpg, 0x00, 0x00, 0x18, 0x06])
  ...
  C('---MEM_COPY BUG BUG BUG---')		# 4.9
  # fSendCmd('MEM_COPY', 0, eeprogpg<<16, 0x4000)

  fSendCmd('MEM_COPY', 0, eeprogpg<<16, 0x0000)
  fGoodCmd()
  fCheckEcho(oSeq, 'MEM_COPY', 0x00,
		[0x00, 0x00, 0x00, 0x00, 0x00, eeprogpg, 0x00, 0x00, 0x00, 0x00])
  fCheckCmdCounts(oSeq)
...
# TBD: Preconditions:
  ...
  fBasicCommand(oSeq)
  fMemory(oSeq)
  fMemoryExtend(oSeq)		# GAM courtesy JRH 2018-11-02
  fMacros(oSeq)
  if (host!='MRO') and (host != 'Lucy') :	# not supported on CRISM or LLORRI
    fSafing(oSeq)
